...
class ImageProcessor(object):
    ...
    @staticmethod
    def find_steering_angle_by_line(img, last_steering_angle, debug = False):
        steering_angle = 0.0
    ...
    @staticmethod
    def find_steering_angle_by_color(img, last_steering_angle, debug = False):
        global isBack
        b, g, r      = cv2.split(img)
        image_height = img.shape[0]
        image_width  = img.shape[1]
        camera_x     = image_width / 2
        image_sample = slice(0, int(image_height * 0.18))
        ...
        if np.count_nonzero(_target == 255) > 100: 
            isBack = None

        if len(tracks_seen) == 0:
            if np.count_nonzero(_target == 255) < 100: 
                #print("count %d" %(np.count_nonzero(_target == 255)))
                isBack = True
            return 0.0

        _y, _x = np.where(_target == 255)
        px = np.mean(_x)
        #isHorizontal = np.count_nonzero(np.mean(_target, axis = 1) > 150) > 5
...
class AutoDrive(object):
    ...
    STEERING_PID_Ki             = 0.0001
    STEERING_PID_Kd             = 0.0125
    STEERING_PID_max_integral   = 10
    THROTTLE_PID_Kp             = 0.075
    THROTTLE_PID_Ki             = 0.005
    THROTTLE_PID_Kd             = 0.02
    THROTTLE_PID_max_integral   = 2.0
    ...
    def on_dashboard(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        normaliztion = self.normaliztion(min(max(ImageProcessor.rad2deg(steering_angle), -Car.MAX_STEERING_ANGLE), Car.MAX_STEERING_ANGLE));

        diff = abs(normaliztion - self.normaliztion(ImageProcessor.rad2deg(last_steering_angle)));
        diffAndle = (1 - (abs(0.5 - normaliztion) / 0.5) * 0.7)
        #logit("steering angle: %0.2f, last steering angle: %0.2f" % (ImageProcessor.rad2deg(steering_angle), ImageProcessor.rad2deg(last_steering_angle)))
        #logit("diff = %0.4f" %(diffAndle))
        if diff < 0.005 :
        ...
        elif diff < 0.07  and self.DEFAULT_SPEED > 3 :
            self.DEFAULT_SPEED *= 0.95;
        elif diff < 0.1 and self.DEFAULT_SPEED > 2.5:
            self.DEFAULT_SPEED *= 0.9;
        elif diff < 0.2 and self.DEFAULT_SPEED > 2.3:
            #logit("diff = %0.4f" %(diff))
            self.DEFAULT_SPEED *= 0.85;
        elif diff < 0.3 and self.DEFAULT_SPEED > 2:
            #logit("diff = %0.4f" %(diff))
            self.DEFAULT_SPEED *= 0.8;
        elif diff < 0.4 and self.DEFAULT_SPEED > 1.7:
            #logit("diff = %0.4f" %(diff))
            self.DEFAULT_SPEED *= 0.75;
        elif diff < 0.5 and self.DEFAULT_SPEED > 1.5:
            #logit("diff = %0.4f" %(diff))
            self.DEFAULT_SPEED = 1.5;
        #else :
        #    logit("diff = %0.4f" %(diff))
        #    self.DEFAULT_SPEED = 0;
       # logit("max speed = %0.2f" %(max_speed * diffAndle))
        if self.DEFAULT_SPEED < 0: 
            self.DEFAULT_SPEED = 0.5
        if self.DEFAULT_SPEED > (max_speed * diffAndle): 
            self.DEFAULT_SPEED = max_speed * diffAndle
        self._throttle_pid.assign_set_point(self.DEFAULT_SPEED)
        ...
        if isBack == None :
            ...
            print("back %0.2f" %(-ImageProcessor.rad2deg(sum(self._steering_history)/self.MAX_STEERING_HISTORY)))
            self._car.control(ImageProcessor.deg2rad(-ImageProcessor.rad2deg(sum(self._steering_history)/self.MAX_STEERING_HISTORY) * 1.8), -0.8);

    def normaliztion(self, value):
        max_angle = 40
        min_angle = -40
        return (value - min_angle) / (max_angle - min_angle)
...
class Car(object):
    ...
    def on_dashboard(self, dashboard):
       ...
       # last_steering_angle = np.pi/2 - float(dashboard["steering_angle"]) / 180.0 * np.pi
        last_steering_angle = ImageProcessor.deg2rad(float(dashboard["steering_angle"]))
